...
def appointment_verify(Id):
    conn = pymysql.connect(host='34.129.105.0', user='Team27', password='Team_27_yu', db='team27', port=3306,
                           charset='utf8')
    cur = conn.cursor()

    cur.execute('SELECT App_Time, App_Date FROM Appointment WHERE Appointment.Id = %s'%Id)
    ...
    if result_date is None and result_time is None:
        ...
        if diff <= 10:
            ...
            cur.execute('UPDATE Appointment SET Check_In = 1 WHERE Appointment.Id = %s'%Id)
        if diff >= 45:
            cur.execute('UPDATE Appointment SET Check_In = 0 WHERE Appointment.Id = %s' % Id)
